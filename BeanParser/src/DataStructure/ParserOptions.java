...
public final class ParserOptions {
	...
	public ParserOptions(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			/*
			 * if (pair[0].equals("training-k")) { trainK =
			 * Integer.parseInt(pair[1]); } if (pair[0].equals("loss-type")) {
			 * lossType = pair[1]; }
			 */
			...
			/*
			 * if (pair[0].equals("decode-type")) { decodeType = pair[1]; } if
			 * (pair[0].equals("format")) { format = pair[1]; } if
			 * (pair[0].equals("relational-features")) { useRelationalFeatures =
			 * pair[1].equals("true") ? true : false; } if
			 * (pair[0].equals("discourse-mode")) { discourseMode =
			 * pair[1].equals("true") ? true : false; } if
			 * (pair[0].equals("confidence-estimation")) { confidenceEstimator =
			 * pair[1]; } if (pair[0].equals("rankEdgesByConfidence")) {
			...
			if (pair[0].equals("filterarc")) {
				filterarc = pair[1].equals("true") ? true : false;
			}
		}

		try {
			File tmpDir = new File(System.getenv("CODEDATA") + File.separator
					+ "tmp");
			...
			/*
			 * if (null != testfile) { testforest = File.createTempFile("test",
			 * ".forest"); testforest.deleteOnExit(); }
			 */
